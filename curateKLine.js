...
async function readFiles() {
  try {
    ...
    kLineData.forEach((entity) => {
      if (entity.speed === 0) {
        if (vehicleStoppedArrayK.includes(entity.vehicle_id)) {
          vehicleStoppedEntitiesK.forEach((stoppedEntity) => {
            ...
            if (stoppedEntity.vehicle_id === entity.vehicle_id) {
              ...
              if (
                entity.latitude === stoppedEntity.latitude &&
                entity.longitude === stoppedEntity.longitude
              ) {
                // vehicle is still at the same stop, do nothing
              } else {
                // vehicle has moved but speed is still 0
                ...
                for (let i = traversedKLineData.length - 1; i >= 0; i--) {
                  if (traversedKLineData[i].vehicle_id === entity.vehicle_id) {
                    ...
                    tempTimeSinceLastUpdate = getTimeDifferenceInSeconds(
                      traversedKLineData[i].timestamp,
                      entity.timestamp
                    );
                    tempDistanceMoved = getDistanceInFeet(
                ...
                let calculatedSpeed =
                  (tempDistanceMoved / tempTimeSinceLastUpdate) * 0.3048; // speed in meters per second

                if (calculatedSpeed > 3.12928) {
                  // 3.12928 m/s = 7 mph
                  ...
                  if (tempOriginalEntity.direction_id === 0) {
                    // outbound
                    let stopFound = false;
                    kLineStopsOutbound.forEach((stop) => {
                      if (
                        isWithinDistance(
                          tempOriginalEntity.latitude,
                          tempOriginalEntity.longitude,
                      ...
                      kLineIntersectionStops.forEach((intersectionStop) => {
                        if (
                          isWithinDistance(
                            tempOriginalEntity.latitude,
                            tempOriginalEntity.longitude,
                            intersectionStop.location.latitude,
                            intersectionStop.location.longitude,
                            500
                          )
                        ) {
                          tempOriginalEntity.atIntersection = true;
                          tempOriginalEntity.intersectionCrossStreet =
                            intersectionStop.stop_name;
                  ...
                  } else if (tempOriginalEntity.direction_id === 1) {
                    // inbound
                    let stopFound = false;
                  ...
                  // remove from array
                  vehicleStoppedEntitiesK = vehicleStoppedEntitiesK.filter(
                    (vehicle) => vehicle !== stoppedEntity
                  );
                }
              }
            }
      ...
      } else if (entity.speed > 3.12928) {
        // 3.12928 m/s = 7 mph
        if (vehicleStoppedArrayK.includes(entity.vehicle_id)) {
          // vehicle no longer stopped, remove from array
          vehicleStoppedArrayK = vehicleStoppedArrayK.filter(
            (vehicle) => vehicle !== entity.vehicle_id
          );
        ...
        // vehicle is moving less than 7 mph
        if (vehicleStoppedArrayK.includes(entity.vehicle_id)) {
          vehicleStoppedEntitiesK.forEach((stoppedEntity) => {
            if (stoppedEntity.vehicle_id === entity.vehicle_id) {
              // calculate speed since last data update
      ...
      if (entity.direction_id === 0) {
        ...
        if (
          ...
          if (
            tripIdStartMapOutbound.has(entity.trip_id) &&
            !tripIdEndMapOutbound.has(entity.trip_id)
          ) {
            tripIdEndMapOutbound.set(entity.trip_id, entity.timestamp);
          }
        }
      } else if (entity.direction_id === 1) {
        // inbound
        if (
          isWithinDistance(
            entity.latitude,
        ...
        if (
          ...
          // Embarcadero Station
          if (
            tripIdStartMapInbound.has(entity.trip_id) &&
            !tripIdEndMapInbound.has(entity.trip_id)
          ) {
      ...
      tempArrayFullTrips = [
        ...tempArrayFullTripsOutbound,
        ...tempArrayFullTripsInbound,
      ];
      // ----------------------- End of tripId logic -----------------------
    });
